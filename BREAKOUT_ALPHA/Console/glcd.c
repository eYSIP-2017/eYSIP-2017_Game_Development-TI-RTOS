...
unsigned char i,q;
unsigned char j, block_x, x_prev, y_prev, p, holder;
uint32_t latency;
uint32_t pinName, baseName;
unsigned char tempSmall[8];
unsigned char tempLarge[32];
unsigned int flag;
/*
...
void displaySmallText(char *text, unsigned char page, unsigned char cursor)
{
    unsigned char i = 0, j = 0, k = 0;
    for(i = 0; i < strlen(text); i++)
    {
