...
\begin{itemize}
  ...
  \end{lstlisting}
  \item \textbf{{\small updateOutput()}} Task: \\ \qquad \qquad Handles GLCD display and LED blink outputs. Also has an internal state machine(implemented using the switch case construct) running as shown in the following code snippet:
  \begin{lstlisting}[basicstyle = \small, language = C]
        switch(vm_mode){
        case INITM:
            // Resets various variables.
